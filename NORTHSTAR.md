...
**Step 1 — v5: the 30-minute live-pull verification (the one sanctioned action).** Run the real Steam + IGDB pull on Yovan's machine; fix whatever auth/fetch/appID-matching bugs surface (code that has never run once usually has some). Success = Yovan's actual backlog in the local DB.
...
- Portfolio positioning polish (Yovan declared Compass explicitly not a portfolio artifact).
