...
- **Build/test commands:** the .NET 10 SDK scaffolded the solution as `Compass.slnx` (new XML format), not `Compass.sln`. Use `dotnet build` / `dotnet test` with **no filename** (auto-discovers `Compass.slnx`) or pass `Compass.slnx` explicitly. Run from `C:\Agent Projects\Compass`. (Anywhere this plan says `Compass.sln`, read it as `Compass.slnx`.)
